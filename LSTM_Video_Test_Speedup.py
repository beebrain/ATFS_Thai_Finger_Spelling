...
def VideoProcess(videoFile, startFrame, endFrame ,size=64):
    cameraObj,totalFrame = intVideo(videoFile)
...
if __name__ == "__main__":
    ...
    for indexTest in range(nrow_test):
        ...
        listImage_frame = VideoProcess(fileVideo_read, 1,-1)
        signSequenceAlphabet_feature = imageseqPredict(CNN_model,listImage_frame)
        class_seq_all = signSequenceAlphabet_feature['Predict_seq']
        ...
        for listAlphabetFrame in listofAlphabetFrames:
            ...
            class_seq_cut = class_seq_all[start-1:stop,:,:]  ## example Frame index 54 = numpy index 53

            class_seq = np.transpose(class_seq_cut,(1,0,2))
